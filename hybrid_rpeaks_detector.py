...
class Config:
    ...
    RR_DEVIATION    = 0.40   # max fractional deviation from running avg
    ...
    MORPH_CORR_MIN  = 0.65   # stricter morphology correlation
...
class RuleBasedFilter:
    ...
    def apply(self, ecg_bp: np.ndarray,
        ...
        # ── Rule 3: Combined energy + sharpness trap ─────────────────
        ...
        # ── Rule 4: Morphology correlation with template ─────────────
...
def compute_sqi(ecg_bp, peaks, fs):
    if len(peaks) < 3:
        return 0.7 
...
class HybridRPeakDetector:
    ...
    def detect(self, ecg_raw: np.ndarray) -> tuple:
        info = {}
        ecg_clean = self.preprocessor.process(ecg_raw)
        candidates, ecg_bp, mwi = self.pan_tompkins.detect(ecg_clean)
        
        if len(candidates) < 2: return candidates, info

        baseline = AdaptiveBaseline(self.fs)
        safe_thresh = max(np.max(np.abs(ecg_bp)) * 0.3, np.mean(np.abs(ecg_bp)) * 2.0)
        
        valid_inits = 0
        prev_init = -1
        for cand in candidates:
            if valid_inits >= 3: break
        ...
        for ci, cand in enumerate(candidates):
            passed, _ = self.rule_filter.apply(ecg_bp, cand, prev_verified, baseline)
            ...
            if strict_mode:
                if abs(ecg_bp[cand]) < 0.5 * baseline.amplitude_avg:
                    continue

            passed_candidates.append(int(cand))

            try:
                feat = self.feat_extractor.extract(ecg_bp, np.asarray(passed_candidates, dtype=int), baseline, len(passed_candidates) - 1)
                embed = self.cnn.embed(ecg_bp, cand)
                combined_feat = np.concatenate([feat, embed])
            except Exception:
                continue

            if self._is_trained and self.model is not None:
                prob = float(self.model.predict_proba([combined_feat])[0, 1])
                if prob < ml_threshold: continue

            if verified and (cand - verified[-1]) < self.rule_filter.rr_min: continue
            
            verified.append(int(cand))
            rr = (cand - prev_verified) if prev_verified >= 0 else None
            
            recent_peaks = verified[-5:] if len(verified) >= 5 else verified
            current_sqi = compute_sqi(ecg_bp, recent_peaks, self.fs)
        ...
        final_peaks = self.post_processor.process(np.array(verified, dtype=int), ecg_bp, baseline)
        return final_peaks, info

        # ── Initialise adaptive baseline ──────────────────────────────
        baseline = AdaptiveBaseline(self.fs)

        # Safe warm-start baseline from prominent early beats only
        safe_thresh = max(
            np.max(np.abs(ecg_bp)) * 0.3,
        ...
        # ── Streaming funnel: Rules -> Features -> ML -> Accept/Update ──
        verified = []
        prev_verified = -1
        rule_reasons = {}
        probs = []
        n_feature_extracted = 0
        n_pass_rules = 0
        passed_candidates = []

        for ci, cand in enumerate(candidates):
            passed, reason = self.rule_filter.apply(ecg_bp, cand, prev_verified, baseline)
            ...
            try:
                ...
                n_feature_extracted += 1
            except Exception:
                rule_reasons[ci] = 'feature_extract_error'
                continue
            ...
            if self._is_trained and self.model is not None:
                ...
                if prob < Config.ML_THRESHOLD:
                    rule_reasons[ci] = f'ml_reject_{prob:.2f}'
                    probs.append(prob)
                    continue

            probs.append(prob)
            # Final refractory safety check (rules + ML already passed)
            if verified and (cand - verified[-1]) < self.rule_filter.rr_min:
                rule_reasons[ci] = 'final_refractory_violation'
                continue
            verified.append(int(cand))
            rr = (cand - prev_verified) if prev_verified >= 0 else None
            baseline.update(ecg_bp, cand, rr)
        ...
        info['n_after_rules'] = n_pass_rules
        info['n_feature_extracted'] = n_feature_extracted
        info['probs'] = np.array(probs, dtype=np.float32)
        info['n_after_ml'] = len(verified)

        if verbose:
            print(f"  [Rules] {len(candidates) - info['n_after_rules']} rejected → {info['n_after_rules']} remaining")
            if self._is_trained and self.model is not None:
                print(f"  [ML]   accepted {len(verified)}")
            else:
                print("  [ML]   No model loaded — using rule-passed peaks")

        # ── Stage 7: Post-processing ───────────────────────────────────
        verified = self.post_processor.process(np.array(verified, dtype=int), ecg_bp, baseline)
...
def extract_training_data(ecg_list: list,
    ...
    """
    Extract 32-Dim features (Rules ONLY) with SQI weights.
    """
    ...
    for subj_idx, (ecg_raw, labels) in enumerate(zip(ecg_list, label_list)):
        ...
        if isinstance(labels, np.ndarray):
            expert_peaks = labels
            for ci, cand in enumerate(candidates):
                min_dist = int(np.min(np.abs(expert_peaks - cand))) if len(expert_peaks) > 0 else 9999
                label = 1 if min_dist <= MATCH_TOL else 0

                # --- NEW SQI & WEIGHT LOGIC ---
                recent_cands = candidates[max(0, ci-5):ci+1]
                sqi = compute_sqi(ecg_bp, recent_cands, fs)
                weight = 0.3 if sqi < 0.4 else 1.0

                try:
                    # ONLY extracting the 32 features here. NO CNN!
                    feat = detector.feat_extractor.extract(ecg_bp, candidates, baseline, ci)
                    all_X.append(feat) 
                    all_y.append(label)
    ...
    if verbose:
        print(f"\nTotal: {len(y)} candidates | {(y==1).sum()} true R | {(y==0).sum()} false P")
...
if __name__ == '__main__':
    ...
    for i in range(min(3, len(peaks))):
        ...
        print(f"    Peak {i+1} @ sample {peaks[i]:4d} | "
              f"{len(feat)} engineered features")
    ...
    print(f"    Feature shape: {X_tr.shape}  (32 engineered)")
    print()
